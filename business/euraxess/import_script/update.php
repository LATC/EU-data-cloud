...
$query_start_date = "SELECT timestamp FROM statistics WHERE ID = (SELECT MAX(ID) FROM statistics)";
$start_date_query = mysql_query($query_start_date);
$start_date = mysql_fetch_array($start_date_query);
$time_start = strtotime($start_date[timestamp]);


echo "Updating fellowships \n";
$ch = curl_init();
...
for ($j=1;$j<=$ids_array[0];$j +=1){
        ...
        foreach ($ids_array[$j] as $fellowship_id){
            $check_id = "SELECT ID FROM fellowship WHERE ID = '$fellowship_id'";
...
$career_stage_array = explode(",", $career_stage[0]);
...
$values[2][0] = mysql_escape_string($values[2][0]);
$values[3][0] = mysql_escape_string($values[3][0]);
...
if ($select_career_a==FALSE){
    ...
    $query8 = "UPDATE fellowship SET career_stage_ID = '$career_stage_ID' WHERE ID = '".$fellowship_id."'
        ";
    mysql_query($query8);
}
else{
