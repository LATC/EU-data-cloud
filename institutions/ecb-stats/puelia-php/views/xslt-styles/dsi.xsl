...
<xsl:template match="result" mode="style">
	...
	<link rel="stylesheet" href="/css/dsi.css" type="text/css" />
	<xsl:comment>
...
<xsl:template match="*" mode="graphRow">
	...
	<xsl:choose>
		...
		<xsl:when test="@datatype = 'integer' or @datatype = 'decimal' or @datatype = 'int' or @datatype = 'float' or @datatype = 'double'">
			...
			<xsl:variable name="showpiechart">
				<xsl:apply-templates select="." mode="showpiechart">
					<xsl:with-param name="values" select="$properties" />
					<xsl:with-param name="distinctValues" select="$distinctValues" />
				</xsl:apply-templates>
			</xsl:variable>
			<xsl:if test="$showpiechart = 'true'">
				...
				<tr>
					...
					<td class="piechart" id="piechart{generate-id(.)}">
						<script type="text/javascript">
						$('#piechart<xsl:value-of select="generate-id(.)"/>').sparkline([<xsl:value-of select="$grouped" />], { type: 'pie',  height:'100px'});
					</script>
					</td>
				</tr>
			</xsl:if>
...
<xsl:template match="*" mode="showpiechart">
...
<xsl:template match="/result/primaryTopic" mode="content" priority="10">
	<header>
		<p id="openSearch">Show Search Form</p>
    <h1><a href="{@href}"><xsl:apply-templates select="." mode="name" /></a></h1>
