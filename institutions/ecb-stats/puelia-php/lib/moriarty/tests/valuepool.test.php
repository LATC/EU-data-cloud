...
class ValuePoolTest extends PHPUnit_Framework_TestCase {
  ...
  function test_get_candidate_values_posts_query() {
    ...
    $fake_request_factory->register('GET', "http://example.org/store/services/sparql?query=PREFIX+p%3A+%3Chttp%3A%2F%2Fpurl.org%2Fvocab%2Fvalue-pools%2Fschema%23%3E+CONSTRUCT+%7B%3Ctranscripts%3E+p%3Avalue+%3Fv+.+%7D+WHERE+%7B+%3Ctranscripts%3E+p%3Avalue+%3Fv+.+%7D+LIMIT+5" . "&output=rdf", $fake_request );
