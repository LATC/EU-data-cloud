...
* Certain observation values are represented as `-`, e.g. see [dataset] (epp.eurostat.ec.europa.eu/NavTree_prod/everybody/BulkDownloadListing?sort=1&file=data%2Fearn_mw_cur.tsv.gz). We assume these values to be 0 and so replace all such values with 0 in RDF. 