...
class SparqlServiceTest extends SparqlServiceBaseTest {
  ...
  function test_describe_single_uri_uses_credentials() {
    ...
    $fake_request_factory->register('GET', "http://example.org/store/services/sparql?query=" . urlencode("DESCRIBE <http://example.org/scooby>") . "&output=rdf", $fake_request );
  ...
  function test_describe_multiple_uris_uses_credentials() {
    $fake_request_factory = new FakeRequestFactory();
    $fake_request = new FakeHttpRequest( new HttpResponse() );
    $fake_request_factory->register('GET', "http://example.org/store/services/sparql?query=" . urlencode("DESCRIBE <http://example.org/scooby> <http://example.org/shaggy> <http://example.org/velma>") . "&output=rdf", $fake_request );
  ...
  function test_describe_to_triple_list_uses_credentials() {
    $fake_request_factory = new FakeRequestFactory();
    $fake_request = new FakeHttpRequest( new HttpResponse() );
    $fake_request_factory->register('GET', "http://example.org/store/services/sparql?query=" . urlencode("DESCRIBE <http://example.org/subj>") . "&output=rdf", $fake_request );
  ...
  function test_graph_uses_credentials() {
    $fake_request_factory = new FakeRequestFactory();
    $fake_request = new FakeHttpRequest( new HttpResponse() );
    $fake_request_factory->register('GET', "http://example.org/store/services/sparql?query=" . urlencode('construct {?s ?p ?o } where { ?s ?p ?o .}') . "&output=rdf", $fake_request );
